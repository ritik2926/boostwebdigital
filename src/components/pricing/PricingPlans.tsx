...
export function PricingPlans() {
  return (
    <section id="plans" className={cn("relative", SECTION_PADDING.spacious)}>
      <Container>
        ...
        <RevealGroup
          ...
          className={cn(STACK.subToContent, "grid grid-cols-1 items-start lg:grid-cols-3", GRID_GAP.default)}
        >
          {PLANS.map((plan) => (
            <RevealItem as="li" key={plan.id} className={plan.mobileOrder}>
              <div className="relative h-full">
                ...
                >
                  ...
                  <div className="mt-2 flex flex-wrap items-baseline gap-x-2">
                    <span className="font-display text-2xl font-bold tabular-nums text-white">{plan.monthly}</span>
                    <span className="text-sm text-white/40">· {plan.annual}</span>
                  </div>
