...
**Flagged conflict, confirmed as the actual convention (2026-09-18):** every "vertical/specialty × SEO-type-service" page built so far has gone **flat at root**, not nested under this doc's `/{vertical}-marketing/{vertical}-{service}/` or `/{specialty}-marketing/{specialty}-seo/` formulas — five times now, on purpose, each one: `/healthcare-seo/` (not `/healthcare-marketing/healthcare-seo/`), `/healthcare-reputation-management/`, `/healthcare-social-media-management/`, `/medical-website-design/`, and `/hair-restoration-seo/` (not `/hair-restoration-marketing/hair-restoration-seo/` — this one was actually built nested first, then corrected same-day to match the other four). Five instances is a real, established site convention, not a one-off exception — this doc's nested slug formulas for this page shape are the ones that need to change, not the site. Until that reconciliation happens, **build any future `{specialty}-seo/`, `{specialty}-website-design/`, `{specialty}-reputation-management/`, etc. page flat at root**, matching the five live pages above, not this doc's nested formula table.
